...
public class ShopService {
    ...
    public String donate(long playerId, int multiplier) {
        ...
        long amount = 300_000L * safeMultiplier;
        if (!player.spendCash(amount)) {
            return "기부금 부족 · 필요 금액 " + amount + "원";
        }
        player.addDonation(amount);
        int reputationGain = Math.toIntExact(amount / 300_000L);
        player.addReputation(reputationGain);
        refreshTitle(player);
        // 최근 기록에는 이번 거래의 금액과 평판 변화만 남긴다.
        // 계속 변하는 누적 기부액은 기부 패널에서 현재값을 직접 보여 준다.
        saveRecord(player, RecordType.DONATION, "기부", -amount, reputationGain, null, null);
    ...
    public String buyLuxuryItem(long playerId, String itemKey) {
        ...
        saveRecord(player, RecordType.BUILDING_BUY, "사치품 구매", -item.price(), reputationGain, item.name(), "30만원당 평판 1.5 · 소수점 버림 · 1회 구매");
